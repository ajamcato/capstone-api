...
const EventEntry = require('../models/Event')
...
router.get('/events', (req, res, next) => {
  EventEntry.find()
    .then(events => {
      // `Event` will be an array of Mongoose documents
      // we want to convert each one to a POJO, so we use `.map` to
      // apply `.toObject` to each one
      return events.map(event => event.toObject())
    })
    // respond with status 200 and JSON of the Event
...
// SHOW
// GET /Event/5a7db6c74d55bc51bdf39793
router.get('/events/:id', (req, res, next) => {
  // req.params.id will be set based on the `:id` in the route
  EventEntry.findById(req.params.id)
    .then(handle404)
...
router.post('/events', requireToken, (req, res, next) => {
  ...
  EventEntry.create(req.body.event)
...
// UPDATE
// PATCH /Event/5a7db6c74d55bc51bdf39793
...
// DESTROY
// DELETE /Event/5a7db6c74d55bc51bdf39793
